...
class MethodMapper {
    ...
    private void validateParameters() {
        if (method.isVarArgs())
            throw new IllegalStateException(String.format("Invalid varargs method %s in @Accessor interface", method.getName()));
        ...
        if (method.getParameterTypes().length < names.size())
            throw new IllegalStateException(String.format("Not enough arguments for method %s, "
        ...
        if (method.getParameterTypes().length > variables.size())
            throw new IllegalStateException(String.format("Too many arguments for method %s, "
